...
class TreePrinter 
{
	 ...
	 */
	public static void printTree(HuffmanTree.Node root) {
		int maxLevel = TreePrinter.maxLevel(root);

		printNodeInternal(Collections.singletonList(root), 1, maxLevel);
	}

	private static void printNodeInternal(List<HuffmanTree.Node> nodes, int level, int maxLevel) {
		...
		List<HuffmanTree.Node> newNodes = new ArrayList<>();
	...
	private static int maxLevel(HuffmanTree.Node node) {
		if (node == null)
			return 0;
